...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for bzedged is necessary for this upgrade with the "reindex=1" bzedge.conf option. \n' +
                 'There are changes necessary in both bzedge.conf and bitcore-node.json. \n\n' +
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/bzedged')
            }
          }
        }
      };
