...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for zclassicd is necessary for this upgrade with the "reindex=1" zclassic.conf option. \n' +
                 'There are changes necessary in both zclassic.conf and bitcore-node.json. \n\n' +
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/zclassicd')
            }
          }
        }
      };
